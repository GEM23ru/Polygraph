...
MODELS = {
    ...
    # ===== Cohere (1000 запросов/мес, без VPN) =====
    "cohere":         Spec("cohere",         "cohere",     "command-a-03-2025",          "smart",   256000,  ["reasoning", "multilingual"]),
    "cohere-reason":  Spec("cohere-reason",  "cohere",     "command-a-reasoning-08-2025","smart",   256000,  ["reasoning"]),

    # ===== Cloudflare Workers AI (10K нейронов/день, без VPN) =====
    "cf-oss120":      Spec("cf-oss120",      "cloudflare", "@cf/openai/gpt-oss-120b",    "smart",   128000,  ["reasoning", "chat"]),
    "cf-oss20":       Spec("cf-oss20",       "cloudflare", "@cf/openai/gpt-oss-20b",     "fast",    128000,  ["fast", "chat"]),
    "cf-kimi":        Spec("cf-kimi",        "cloudflare", "@cf/moonshotai/kimi-k2.6",   "smart",   256000,  ["reasoning", "long"]),
    "cf-glm":         Spec("cf-glm",         "cloudflare", "@cf/zai-org/glm-5.2",        "smart",   128000,  ["reasoning"]),
    "cf-qwen":        Spec("cf-qwen",        "cloudflare", "@cf/qwen/qwen3-30b-a3b-fp8", "balanced",128000,  ["coding", "chat"]),

    # ===== GitHub Models (требует GitHub Personal Access Token, бесплатно) =====
    "gh-gpt5":        Spec("gh-gpt5",        "github",     "openai/gpt-5",               "smart",   128000,  ["reasoning", "top"]),
    "gh-gpt5-mini":   Spec("gh-gpt5-mini",   "github",     "openai/gpt-5-mini",          "fast",    128000,  ["fast", "chat"]),
    "gh-o3":          Spec("gh-o3",          "github",     "openai/o3",                  "smart",   200000,  ["reasoning", "top"]),
    "gh-llama4":      Spec("gh-llama4",      "github",     "meta/Llama-4-Maverick-17B-128E-Instruct-FP8", "smart", 128000, ["reasoning"]),
}
...
class CloudflareProvider(BaseProvider):
    """Cloudflare Workers AI — текстовые модели через REST API.
    Ключ + account_id одни и те же, что и для генерации картинок."""
    name = "cloudflare"
    ...
    def ok(self) -> bool:
        return bool(self.api_key and self.account_id
                    and "your-key-here" not in self.api_key)
    def call(self, mid, sys, p, t=0.7, mt=2048):
        ...
        resp = _r.post(url,
                       ...
                       json={"messages": msgs, "max_tokens": mt, "temperature": t},
                       timeout=60)
        data = resp.json()
        if not data.get("success"):
            errs = data.get("errors", [])
            raise RuntimeError(f"Cloudflare error: {errs}")
        # Текстовый ответ в result.response
        result = data.get("result", {})
        return result.get("response", "") or result.get("output", "")


class GitHubModelsProvider(BaseProvider):
    """GitHub Models — OpenAI-совместимый API на https://models.github.ai.
    Требует GitHub Personal Access Token (PAT) с правом models:read."""
    name = "github"
    def __init__(self, key=""):
        super().__init__(key)
        self._c = None
    ...
    def call(self, mid, sys, p, t=0.7, mt=2048):
        ...
        if sys: msgs.append({"role": "system", "content": sys})
        msgs.append({"role": "user", "content": p})
        r = self.c.chat.completions.create(model=mid, messages=msgs,
...
class Polygraph:
    ...
    def __init__(self):
        ...
        # Cloudflare принимает 2 переменные: ключ и account_id
        cf_key = os.environ.get("CLOUDFLARE_API_TOKEN", "")
        cf_acc = os.environ.get("CLOUDFLARE_ACCOUNT_ID", "")
        self.providers["cloudflare"] = CloudflareProvider(cf_key, cf_acc)
